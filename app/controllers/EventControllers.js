...
module.exports.read = (req, res) => {
    const Event_name = req.body.Event_name;
        Events.findOne({"Event_name":Event_name})
        .then(e=>{
            res.status(200).json(e);
        })
        .catch(err=>{
            res.status(500).json({msg:"someting went worng, please try again later"})
        })
}
//update Event
module.exports.update = (req, res) => {
    const Event_name = req.params.Event_name;
    Events.findByIdAndUpdate(Event_name)
    .then(e =>{
        const params={
            Event_name:req.body.Event_name,
            Event_studentcoordinator:req.body.Event_studentcoordinator,
...
module.exports.delete = (req, res) => {
  const Event_name = req.params.Event_name;
  Events.findByIdAndDelete(Event_name).then(result => {
